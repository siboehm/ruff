...
impl Configuration {
    pub fn into_settings(self, project_root: &Path) -> Result<Settings> {
        ...
        let line_width = self.line_width.unwrap_or_default();
    ...
    pub fn from_options(options: Options, project_root: &Path) -> Result<Self> {
        ...
        let line_width = {
            if options.line_length.is_some() {
                warn_user_once!("The option `line-length` has been renamed to `line-width` to emphasize that the limit is the display width of a line and not the number of characters. Use `line-width` instead.");
            }
